...
export const PARAM_KEYS = [
  ...
  "coupon", // Rimo pricing-profile code (eros1 / passion1). NOT an experiment — a payload, like sc_order. Carried so a discount promised on a lander survives an intermediate page: the insert-card quiz's `lander` arm hands off to /eros or /passion, and without this the coupon would be dropped on that hop and the visitor would meet full pack pricing at checkout after being promised $1. withAttribution() is set-if-absent, so a coupon already present on a handoff URL is never overwritten. Self-cleaning in practice: captureAttribution() replaces the whole snapshot whenever a later landing URL carries any PARAM_KEYS, so a stale coupon does not survive a subsequent ad click.
